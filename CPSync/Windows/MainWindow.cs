...
namespace CPSync.Windows;

public class MainWindow : Window, IDisposable
{
    private readonly string iconImagePath;
    private readonly CPSync plugin;

    // We give this window a hidden ID using ##.
    // The user will see "My Amazing Window" as window title,
    // but for ImGui the ID is "My Amazing Window##With a hidden ID"
    public MainWindow(CPSync plugin, string iconImagePath)
    ...
    public override void Draw()
    {
        ...
        using (var child = ImRaii.Child("SomeChildWithAScrollbar", Vector2.Zero, true))
        {
            ...
            if (child.Success)
            {
                ...
                var goatImage = CPSync.TextureProvider.GetFromFile(iconImagePath).GetWrapOrDefault();
                if (goatImage != null)
                {
                ...
                var playerState = CPSync.PlayerState;
                ...
                var iconTexture = CPSync.TextureProvider.GetFromGameIcon(new GameIconLookup(jobIconId)).GetWrapOrEmpty();
                ...
                var territoryId = CPSync.ClientState.TerritoryType;
                if (CPSync.DataManager.GetExcelSheet<TerritoryType>().TryGetRow(territoryId, out var territoryRow))
                {
